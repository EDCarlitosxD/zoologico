...
export class EditRecorridosComponent {
  ...
  ngOnInit(): void {
    ...
    this.recorridoService
      .getHorariosById(parseInt(id))
      .subscribe((data) => (this.horarios = data));

      this.recorridoForm.patchValue({
        titulo: this.recorrido.titulo,
        precio: this.recorrido.precio,
        duracion: this.recorrido.duracion,
        descripcion: this.recorrido.descripcion,
        descripcion_incluye: this.recorrido.descripcion_incluye,
        descripcion_importante_reservar: this.recorrido.descripcion_importante_reservar,
        img_recorrido: this.recorrido.img_recorrido,
      });
  ...
  recorrido: IRecorrido = {
    ...
    img_recorrido: '',
    duracion: undefined as unknown as number,
    valoracion: undefined as unknown as number,
    estado: undefined as unknown as number,
  };
  horario: HorarioTour = {
    id: undefined as unknown as number,
    fecha: '',
  ...
  toggleActive(horario: HorarioTour) {
    ...
    this.recorridoService
      .actualizarEstadoHorario(horario.disponible, horario.id)
      .subscribe((data) => console.log(data));

    console.log(
      `${horario.fecha} is now ${horario.disponible ? 'active' : 'inactive'}`
    );
  }
  ...
  agregarHorario() {
    ...
    if (
      !this.horario.fecha ||
      !this.horario.horario_fin ||
      !this.horario.horario_inicio
    ) {
  ...
  async actualizar(event: Event) {
    ...
    if (typeof duracionSegundos === 'number' && !isNaN(duracionSegundos)) {
      const formatedTime =
        this.recorridoService.formatDurationToTime(duracionSegundos);
      this.recorrido!.duracion = formatedTime;
    }
    // Preparar los datos para guardar
    const dataSave: IRecorridoSave = {
      ...this.recorrido!,
      img_recorrido: this.recorridoForm.get('img_recorrido')?.value,
